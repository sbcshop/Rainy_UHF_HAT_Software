...
"""
1.14" TFT Display Driver for Raspberry Pi
- Initializes SPI and GPIO settings
- Provides methods to send commands, display images, and clear the screen
"""
...
class Lcd_config:
    """
    LCD Configuration Class
    - Handles GPIO and SPI setup
    - Provides methods for writing digital signals and handling backlight PWM
    """
    def __init__(self,spi=spidev.SpiDev(0,0),spi_freq=40000000,rst = 27,dc = 25,bl = 22,bl_freq=1000,i2c=None,i2c_freq=100000):
        import RPi.GPIO      
        self.np=np
        self.RST_PIN= rst
        self.DC_PIN = dc
...
class lcd_display(Lcd_config):
    """
    LCD Display Class
    - Handles LCD initialization, rotation, drawing, and clearing
    """
    width = 240
    height = 135 
    ...
    def rotate(self, rotation):
        ...
        rotation_map = {
            0: (0x60, xWIDTH, xHEIGHT),  # 0°
            1: (0x00, xHEIGHT, xWIDTH),  # 90°
            2: (0xA0, xWIDTH, xHEIGHT),  # 180°
            3: (0xC0, xHEIGHT, xWIDTH)   # 270°
        }
        rotation_value, width, height = rotation_map.get(rotation, (0x00, 240, 135))
        self.command(0x36)
        self.data(rotation_value)
